...
function module.add_host(host_module)
    ...
    local function task_added(event)
        ...
        if task.id == nil then
            task.id = event.source.name .. "/" ..
                          task.name:gsub("%W", "_"):lower();
        end
        task.period = host_module:get_option_period(
                          task.id:gsub("/", "_") .. "_period", "1" .. task.when,
                          60, 86400 * 7 * 53);
...
module:add_item("shell-command", {
    ...
    handler = function(self, filter_host)
        ...
        local row = format_table({
            {title = "Host", width = "2p"}, {title = "Task", width = "3p"},
            {title = "Desc", width = "3p"}, {title = "When", width = "1p"},
            {title = "Last run", width = "20"}
        }, self.session.width);
        local print = self.session.print;
        print(row());
        for host in it.sorted_pairs(filter_host and {[filter_host] = true} or
                                        active_hosts) do
            for _, task in ipairs(module:context(host):get_host_items("task")) do
                print(row({
                    host, task.id, task.name, task.when,
                    task.last and os.date("%Y-%m-%d %R:%S", task.last) or
                        "never"
                }));
            end
        end
